...
export function VendorPage() {
  ...
  const [formData, setFormData] = useState<Omit<Vendor, 'id' | 'created_at'>>({
    ...
    bank_id: '', bank_account_number: '', bank_account_name: '', term_of_payment: '', payable_account_id: '',
    is_active: true
  });
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [vendorRes, bankRes, coaRes] = await Promise.all([
        stakeholdersApi.getVendors(),
        financialsApi.getBanks(),
        financialsApi.getCoas()
      ]);
      setBanks(bankRes.data);
      setCoas(coaRes.data.filter((c: any) => c.account_type === 'Liability')); // AP is Liability

      // Sort by code
      const sorted = vendorRes.data.sort((a: Vendor, b: Vendor) => 
        a.code.localeCompare(b.code)
      );
      setVendors(sorted);
    } catch (error) {
      console.error('Failed to fetch data:', error);
      addToast('error', 'Connection Error', 'Failed to fetch vendor data.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const columns = [
    { header: 'Vendor Code', accessor: 'code' as keyof Vendor, className: 'font-mono text-primary w-24' },
    { 
      header: 'Vendor Name', 
      accessor: (row: Vendor) => (
    ...
    { header: 'NPWP', accessor: 'npwp' as keyof Vendor, className: 'text-textSecondary font-mono text-xs' },
    { 
      header: 'Status', 
      accessor: (row: Vendor) => (
  ...
  const handleAdd = () => {
    setEditingItem(null);
    setFormData({ code: '', name: '', npwp: '', address: '', contact: '', bank_id: '', bank_account_number: '', bank_account_name: '', term_of_payment: '', payable_account_id: '', is_active: true });
    setIsFormOpen(true);
  };
  ...
  const handleDeleteClick = (row: Vendor) => {
    setEditingItem(row);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = { 
      ...
      payable_account_id: formData.payable_account_id || null
    };

    try {
      if (editingItem) {
        await stakeholdersApi.updateVendor(editingItem.id, payload);
        addToast('success', 'Vendor Updated', `Vendor ${formData.code} has been updated.`);
      } else {
        await stakeholdersApi.createVendor(payload);
        addToast('success', 'Vendor Created', `Vendor ${formData.code} has been created.`);
      }
      await fetchData();
      setIsFormOpen(false);
    } catch (error: any) {
  ...
  const confirmDelete = async () => {
    if (editingItem) {
      setIsSaving(true);
      try {
        await stakeholdersApi.deleteVendor(editingItem.id).catch(() => stakeholdersApi.updateVendor(editingItem.id, { is_active: false }));
        addToast('success', 'Vendor Deleted', `Vendor ${editingItem.code} has been removed.`);
        await fetchData();
        setIsDeleteOpen(false);
      } catch (error) {
        addToast('error', 'Delete Failed', 'Could not delete the vendor.');
      } finally {
        setIsSaving(false);
      }
    }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-120px)]">
        <div className="text-textSecondary animate-pulse">Loading vendor data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-textPrimary">Vendor</h1>
      ...
      <div className="flex-1 overflow-hidden min-h-0">
        <DataTable
          ...
          description="Manage supplier information, billing, and accounting integration."
          columns={columns}
          data={vendors}
          searchPlaceholder="Search vendor by code or name..."
          onAdd={handleAdd}
          onEdit={handleEdit}
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingItem ? "Edit Vendor" : "Add New Vendor"} maxWidth="max-w-3xl">
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-textPrimary">Vendor Code <span className="text-danger">*</span></label>
                  <input required type="text" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-textPrimary" placeholder="e.g. VEND-001"/>
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-textPrimary">Vendor Name <span className="text-danger">*</span></label>
                <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-textPrimary" placeholder="e.g. CV Multi Teknik"/>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={formData.contact} onChange={e => setFormData({...formData, contact: e.target.value})} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-textPrimary" placeholder="e.g. Jane Doe / 0812..."/>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-textPrimary">AP Account (Account Payable)</label>
                <select value={formData.payable_account_id} onChange={e => setFormData({...formData, payable_account_id: e.target.value})} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-textPrimary">
                  <option value="">Map to COA...</option>
                  {coas.map(c => <option key={c.id} value={c.id}>{c.account_code} - {c.account_name}</option>)}
                </select>
                <p className="text-[10px] text-textSecondary">Default GL account for vendor invoices.</p>
              </div>

              <div className="p-3 border border-border rounded-lg bg-secondary/5 space-y-3">
                <h4 className="text-xs font-bold text-textPrimary">Vendor Bank Details</h4>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={formData.bank_account_name} onChange={e => setFormData({...formData, bank_account_name: e.target.value})} className="w-full px-3 py-1.5 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-textPrimary" placeholder="e.g. CV Multi Teknik"/>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border mt-6">
            ...
            <button type="submit" disabled={isSaving} className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors disabled:opacity-50">
              <Save className="w-4 h-4" /> {isSaving ? 'Saving...' : 'Save Vendor'}
            </button>
          </div>
        </form>
      </Modal>
